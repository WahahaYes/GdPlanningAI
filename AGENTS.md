...
Prefer `make` targets over executing arbitrary shell commands to preserve context. Check the [Makefile](Makefile) ([Rust Makefile](addons/GdPlanningAI/rust/Makefile)) for available targets before proposing custom commands.

For example, `cd /c/godot/gdplanningai/ && make test-godot` instead of `godot --test`.
