...
## Regression test for the `Go To → Go To` ordering bug.
## Two different `at_target` requirements (pickup and drop) must each get a
## `Go To` immediately before the action that consumes them.
func test_goto_wildcard_orders_two_goto_for_pickup_and_use() -> void:
	var scheduler: GdPAIPlanScheduler = _make_scheduler()
	await get_tree().process_frame

	var pickup_location: GdPAILocationData = GdPAILocationData.new()
	pickup_location.position = Vector3(10.0, 0.0, 5.0)
	add_child_autofree(pickup_location)

	var drop_location: GdPAILocationData = GdPAILocationData.new()
	drop_location.position = Vector3(20.0, 0.0, 15.0)
	add_child_autofree(drop_location)
	...
	var goto_effect: Callable = func(agent: GdPAIBlackboard, _world: GdPAIBlackboard) -> void:
		agent.set_property("at_location", true)
	var pickup_effect: Callable = func(agent: GdPAIBlackboard, _world: GdPAIBlackboard) -> void:
		agent.set_property("held_item", "wood")
	var use_effect: Callable = func(agent: GdPAIBlackboard, _world: GdPAIBlackboard) -> void:
		agent.set_property("dropped", true)
		agent.set_property("held_item", "")
	...
	var actions: Array[Dictionary] = [
		{
			...
			"requirements":
			[
				{"kind": "binding_equals", "binding_name": "held_item", "value": "wood"},
				{"kind": "fact", "fact_name": "at_target", "args": [drop_location]},
			],
			"provisions": []
		},
		{
			...
			"preconditions": [],
			"validity_checks": [],
			"requirements": [{"kind": "fact", "fact_name": "at_target", "args": [pickup_location]}],
			"provisions": [{"kind": "binding", "binding_name": "held_item", "value": "wood"}]
		},
		{
			"name": "GoTo",
			"cost_callable": cost_goto,
	...
	var goals: Array[Dictionary] = [
		{
			...
			"reward": 100.0,
			"desired_state":
			[{"target": "agent", "operation": "equal", "property_name": "dropped", "value": true}]
		}
	]

	var result: Dictionary = await _submit_plan_and_wait(
		scheduler,
		_make_blackboard({"held_item": "", "dropped": false}),
		_make_blackboard(),
		actions,
		goals,
		120,
	...
	assert_true(result["success"], "Plan should succeed with two GoTo actions ordered correctly")
	assert_eq(result["action_chain"].size(), 4, "Plan should have 4 actions")

	# Expected order: GoTo(pickup) -> Pickup -> GoTo(drop) -> Use
	var chain: Array = result["action_chain"]
	assert_eq(chain[0], 2, "First action should be GoTo to pickup location")
	assert_eq(chain[1], 1, "Second action should be Pickup")
	assert_eq(chain[2], 2, "Third action should be GoTo to drop location")
	assert_eq(chain[3], 0, "Fourth action should be Use")
